...
def apply_style(ax, x, labels):
    ...
    for i in range(len(labels)):
        if labels[i] == "Mcts":
            labels[i] = "MCTS Agent"
        elif labels[i] == "Random":
            labels[i] = "Random Agent"
        elif labels[i] == "Inginious":
            labels[i] = "Inginious Agent"
        elif labels[i] == "Greedy":
...
def graph1_win_rate(data, out_dir, opponent):
    ...
    ax.set_title(f"Taux de victoires (%) avec l'{opponent}", fontsize=14, pad=15)
...
def graph2_avg_game_time(data, out_dir, opponent):
    ...
    ax.set_title(f"Temps moyen de jeu avec l'{opponent}", fontsize=14, pad=15)
...
def graph3_avg_turn_time(data, out_dir, opponent):
    ...
    ax.set_title(f"Temps moyen par tour avec l'{opponent}", fontsize=14, pad=15)
...
def graph4_avg_moves(data, out_dir, opponent):
    ...
    ax.set_title(f"Nombre de coups moyen avec l'{opponent}", fontsize=14, pad=15)
